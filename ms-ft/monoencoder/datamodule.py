...
from utils.ir_dataset_utils import get_base
...
def _read_ir_dataset(
    ...
    if data_type == "queries":
        print("Loading queries for dataset: ", dataset.dataset_id())
        ...
        print("Loading qrels for dataset: ", dataset.dataset_id())
        ...
        print("Loading scoreddocs for dataset: ", dataset.dataset_id())
        ...
        print("Done loading scoreddocs.")
    else:
        raise ValueError(f"Error: invalid data_type: {data_type}")
...
class DocStore:
    def __init__(self, docstore: ir_datasets.indices.Docstore, doc_fields: Optional[Iterable[str]]) -> None:
...
class IRDataset:
    ...
    @staticmethod
    def load_runs(ir_datasets: Iterable[ir_datasets.Dataset]) -> Dict[str, pd.DataFrame]:
    ...
    @staticmethod
    def load_qrels(ir_datasets: Iterable[ir_datasets.Dataset]) -> Dict[str, pd.DataFrame]:
...
class MonoEncoderDataset(torch.utils.data.Dataset, IRDataset):
    def __init__(
        self,
        ir_datasets: Iterable[ir_datasets.Dataset],
        sample_size: int,
    ...
    def sample_random(df: pd.DataFrame, non_relevant_size: int) -> pd.DataFrame:
        ...
        non_relevant_samples = non_relevant_df.sample(non_relevant_size).set_index("doc_id")
    ...
    def sample_first(
        ...
        non_relevant_df = non_relevant_df.loc[non_relevant_df.relevance == 0]
        non_relevant_size = min(non_relevant_size, non_relevant_df.shape[0])
        non_relevant_samples = non_relevant_df.iloc[:non_relevant_size].set_index("doc_id")
        return non_relevant_samples

    @staticmethod
    def sample_first_neg(df: pd.DataFrame, non_relevant_size: int, first: bool, fill_up: bool) -> pd.DataFrame:
    ...
    def get_relevant_sample(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.relevant_sampling_strategy == "first":
            ...
            raise ValueError(f"Invalid relevant sampling strategy: {self.relevant_sampling_strategy}")
        rel_samples = rel_df.set_index("doc_id")
        return rel_samples

    def get_non_relevant_sample(self, df: pd.DataFrame, num_relevant_samples: int) -> pd.DataFrame:
        non_relevant_size = self.sample_size - num_relevant_samples
       
        if self.non_relevant_sampling_strategy == "first":
    ...
    def get_sample(self, df: pd.DataFrame) -> Tuple[Dict[str, int], Dict[str, int] | None]:
        ...
        elif self.relevant_sampling_strategy == "topk":
            sample_size = min(self.sample_size, df.shape[0])
            relevant_samples = df.head(sample_size).set_index("doc_id")
        ...
        return sample_df["relevance"].to_dict()
...
class MonoEncoderDataModule(pl.LightningDataModule):
    def __init__(
        self,
        ...
        test_ir_dataset_paths: Optional[Iterable[Union[str, Path]]] = None,
        doc_fields: Optional[Iterable[str]] = None,
        truncate: bool = True,
        max_length: int = 512,
        ...
        self.test_ir_dataset_paths = test_ir_dataset_paths
        self.doc_fields = doc_fields
    ...
    def setup_train(self, ir_datasets: Iterable[ir_datasets.Dataset]) -> None:
        self.train_dataset = MonoEncoderDataset(
    ...
    def setup_test(self, ir_datasets: Iterable[ir_datasets.Dataset]) -> None:
        self.test_datasets = []
        for ir_dataset in ir_datasets:
            self.test_datasets.append(
                MonoEncoderDataset(
                    ...
                    min_num_relevant_samples=0,
                    non_relevant_sampling_strategy="first",
                    relevant_sampling_strategy="first",
                    depth=self.depth,
                    shuffle_docs=self.shuffle_docs,
                    doc_fields=self.doc_fields,
                    load_qrels=False,
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if stage == "test":
            assert self.test_ir_dataset_paths is not None
            datasets = [
                load_ir_dataset(ir_dataset_path)
                for ir_dataset_path in self.test_ir_dataset_paths
            ]
            self.setup_test(datasets)
    ...
    def train_dataloader(self):
        ...
        return torch.utils.data.DataLoader(
            ...
            shuffle=(
                ...
                if isinstance(self.train_dataset, MonoEncoderDataset)
                else False
            ),
    ...
    def collate_and_tokenize(self, samples: Iterable[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        del queries, docs, labels, optimal_labels
        gc.collect()
        return batch
