...
def load_collection(dataset: ir_datasets.Dataset, doc_fields: Optional[List[str]]) -> Iterator[Tuple[str, str]]:
    for doc in tqdm(dataset.docs_iter(), total=dataset.docs_count(), desc="loading collection"):
        if doc_fields:
            contents = " ".join(
...
def main(args=None):
    ...
    for dataset_name in args.ir_datasets:
        ...
        print("Starting indexing with searcher.")

        searcher.index(dataset, args.doc_fields, args.reindex)
        
        print("Done indexing with searcher.")
        ...
        print("Starting creating run file.")
        create_run_file(
            searcher,
            dataset,
            run_file,
